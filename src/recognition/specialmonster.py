class SpecialMonsterHandler:
    ...
    def check_special_monsters(self, left_monsters, right_monsters, winner):
        ...
        for monster_id, config in self.special_monsters.items():
            left_has = (
                left_monsters[str(monster_id)].text().isdigit()
                and int(left_monsters[str(monster_id)].text()) > 0
            )
            right_has = (
                right_monsters[str(monster_id)].text().isdigit()
                and int(right_monsters[str(monster_id)].text()) > 0
            )
