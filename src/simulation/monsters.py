...
class StatusSystem:
    ...
    def apply(self, effect):
        ...
        # 处理效果叠加逻辑
        existing = next(
            (e for e in self.effects if e.type == effect.type), None
        )
    ...
    def _process_dot(self, delta_time):
        ...
        corrupt = next(
            (e for e in self.effects if e.type == BuffType.CORRUPT), None
        )
        if corrupt:
            # 每秒造成伤害
...
class Monster:
    ...
    def move_toward_enemy(self, delta_time):
        ...
        if not self.blocked and self.attack_state == AttackState.等待:
            self.velocity = (
                self.velocity * 7 + norm_direction * self.move_speed
            ) / 8
    ...
    def can_attack(self, delta_time):
        ...
        if self.attack_state == AttackState.前摇:
            if in_range:
                ...
                if (
                    self.attack_time_counter
                    >= self.attack_animation.windup_time
                ):
    ...
    def update(self, delta_time):
        ...
        if (
            ...
            or (self.target.position - self.position).magnitude
            > self.attack_range
        ):
            # 寻找新目标
    ...
    def apply_damage_to_target(self, target, damage) -> bool:
        ...
        if target.take_damage(damage, self.attack_type):
            return True
        debug_print(
            f"{self.name}{self.id} 没有对 {target.name}{target.id}造成伤害"
        )
        return False
...
class 炽焰源石虫(Monster):
    ...
    def apply_damage_to_target(self, target: Monster, damage):
        if super().apply_damage_to_target(target, damage):
            target.element_system.accumulate(
                ElementType.FIRE, self.get_attack_power()
            )
            return True
        return False
        ...
        for m in self.battlefield.alive_monsters:
            if m != self and m.faction != self.faction and m.is_alive:
                ...
                if distance <= explosion_radius:
                    ...
                    # 施加10秒寒冷效果
                    chill = BuffEffect(
                        type=BuffType.CHILL, duration=10, source=self
                    )
...
class 大喷蛛(Monster):
    ...
    def spawn_small(self):
        ...
        self.battlefield.append_monster_name(
            ...
            self.position
            + FastVector(
                random.uniform(-1, 1) * 0.2, random.uniform(-1, 1) * 0.2
            ),
        )
    ...
    def apply_damage_to_target(self, target, damage):
        ...
        if super().apply_damage_to_target(target, damage):
            if self.attack_count % 3 == 0:
                # 施加寒冷效果
                chill = BuffEffect(
                    type=BuffType.CHILL, duration=5, source=self
                )
...
class 庞贝(Monster):
    ...
    def on_extra_update(self, delta_time):
        ...
        if (
            len(targets) > 0
            and (targets[0].position - self.position).magnitude < 0.8
        ):
            if self.ring_attack_counter >= 10.0:
                targets = [
                    t
                    for t in targets
                    if (t.position - self.position).magnitude < 1.4
                ]
                for tar in targets:
    ...
    def apply_damage_to_target(self, target, damage):
        if super().apply_damage_to_target(target, damage):
            ...
            if self.attack_count % 4 == 0:
                dizzy = BuffEffect(
                    type=BuffType.DIZZY, duration=7, source=self
                )
                target.status_system.apply(dizzy)
                debug_print(
                    f"{self.name}{self.id} 眩晕了 {target.name}{target.id}"
                )
            return True
        return False
            ...
            if self.attack_count % 4 == 0:
                ...
                for m in targets:
                    ...
                    if self.apply_damage_to_target(m, damage):
                        ...
                        m.status_system.apply(
                            BuffEffect(
                                type=BuffType.DIZZY, duration=3, source=self
                            )
                        )
            else:
                super().attack(target, gameTime)
...
class 山海众窥魅人(Monster):
    ...
    def on_extra_update(self, delta_time):
        ...
        if self.stage == 1:
            # 蓄力5秒后造成攻击力200%法术伤害
            if (
                self.charging_counter >= 5
                or not self.locked_target.can_be_target()
            ):
                self.stage = 2
            ...
            if self.charging_counter >= 7:
                ...
                for m in self.battlefield.monsters:
                    if m.faction != self.faction and m.can_be_target():
                        ...
                        x = int(
                            math.floor(m.position.x - self.target_pos.x + 0.5)
                        )
                        y = int(
                            math.floor(m.position.y - self.target_pos.y + 0.5)
                        )
                        if abs(x) + abs(y) <= 2:
...
class 萨卡兹链术师(Monster):
    ...
    def attack(self, target, gameTime):
        ...
        for node in taragets:
            ...
            if self.apply_damage_to_target(node.target, dmg):
                ...
                node.target.element_system.accumulate(
                    t, self.get_attack_power() * 0.3
                )
    ...
    def on_death(self):
        ...
        switch_stage = BuffEffect(
            type=BuffType.INVINCIBLE2, duration=1, source=self
        )
...
class 狂躁珊瑚(Monster):
    ...
    def on_extra_update(self, delta_time):
        ...
        if (
            ...
            and abs(round(self.decay_timer - 3.5) - (self.decay_timer - 3.5))
            < 0.001
        ):
    ...
    def on_extra_update(self, delta_time):
        ...
        if self.stage == 0 and self.skill_counter >= 24:
            ...
            if not self.locked_target.can_be_target():
                ...
                dmg = self.calculate_damage(
                    self.locked_target,
                    self.get_attack_power() * 0.4 * delta_time,
                )
                ...
                # 蓄力完成的凋亡损伤
                if (
                    self.locked_target.can_be_target()
                    and self.charging_counter2 >= 8.0
                ):
                    for m in self.get_aoe_targets(self.locked_target):
                        dmg = self.get_attack_power() * 2.2
                        m.element_system.accumulate(
                            ElementType.NECRO_RIGHT, dmg
                        )
                        debug_print(
...
class MonsterFactory:
    ...
    def create_monster(cls, data, faction, position, battlefield):
        ...
        if monster_type in cls._monster_classes:
            m = cls._monster_classes[monster_type](
                data, faction, position, battlefield
            )
            m.on_spawn()
            return m
        else:
