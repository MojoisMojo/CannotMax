...
def get_random_png_frame(png_folder):
    ...
    frame = cv2.imread(
        str(random_png), cv2.IMREAD_UNCHANGED
...
def composite_random_frame():
    # 读取战场背景图
    battlefield = cv2.imread(
        "./tools/battlefield_composite/monster_images/IM-1.png"
    )
    ...
    for i in range(10):
        ...
        frame = get_random_png_frame(
            "./tools/battlefield_composite/monster_images/Arc_Frontliner_Leader-Move"
        )
    ...
    # 保存结果
    cv2.imwrite(
        "./tools/battlefield_composite/battlefield_composite.png", battlefield
    )
...
def crop_to_bounding_box(image):
    ...
    if image.shape[2] < 4:
        ...
        print(
            "警告: 图像不包含alpha通道，无法进行基于透明度的裁切。返回原图。"
        )
        return image
