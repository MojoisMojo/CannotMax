...
def is_continuous_sublist(sub, main):
    return any(
        sub == main[i : i + len(sub)] for i in range(len(main) - len(sub) + 1)
    )
...
def remove_duplicate_subsequences_easy(listdata, threshold=3):
    ...
    reco_last.sort()
    processed_data = [
        listdata[j] for j in range(len(listdata)) if j not in reco_last
    ]
...
def read_and_remove_zeros(filename, MONSTER_NUM=56):
    ...
    data_new = [
        datafull[j] for j in range(len(datafull)) if j not in all_zeros
    ]
...
def do_duplicate(listdata):
    if listdata == []:
        return [], []
    num_data = [
        list(map(int, map(float, i[: MONSTER_NUM * 2]))) for i in listdata
    ]
    np_num_data = np.array(num_data)
    _, remove_list = remove_duplicate_subsequences(np_num_data, threshold=3)
    # print(remove_list)
    result = [
        listdata[j] for j in range(len(listdata)) if j not in remove_list
    ]
...
def view_monster_counts(listdata):
    ...
    num_right = [
        list(map(int, map(float, i[MONSTER_NUM : MONSTER_NUM * 2])))
        for i in listdata
    ]
...
def del_duplicate_by_time(listdata, delete_no_time=True):
    ...
    data_with_time = [
        listdata[j] for j in range(len(listdata)) if j not in no_time
    ]
    ind = 0
    for i in data_with_time:
        if i[-1] not in timedata:
            ...
            print(
                f"{timedata.index(i[-1])}行与{ind}行发现同名截图，文件名：{i[-1]}"
            )
        ind += 1
    if not delete_no_time:
        # 先找到wrongtime元素的原始位置，再从原列表删除
        remaining_after_first = [
            i for i in range(len(listdata)) if i not in no_time
        ]
        second_deleted_original = [
            remaining_after_first[t] for t in wrong_time
        ]
        data_with_time_ok = [
            listdata[j]
        ...
        data_with_time_ok = [
            data_with_time[j]
            for j in range(len(data_with_time))
            if j not in wrong_time
        ]
...
def find_where_from(easydata, floder_path):
    ...
    if from_list != []:
        str_from = "\n".join(
            [i[0] + "第：" + str(i[1]) + "行" for i in from_list]
        )
...
def is_list_true_np(fulllist):
    ...
    right_min_terms = (
        ...
        + ((right_part - 1) * (right_part - 2) * cost_arr[np.newaxis, :, 1])
        / 2
        + 0.01
    ) * valid_right
...
def recognize_review(
    data, img_floder, matched_threshold=0.1, ocr_threshold=0.5
):
    print("正在进行识别数据检查")
    ...
    for idx, row in tqdm.tqdm(
        enumerate(data), total=len(data), desc="Processing rows"
    ):
        ref_row = [0] * (recognize.MONSTER_COUNT * 2)
        try:
...
def process_full(
    ...
    wrong_type_list = []
    newdata, deleted0, ori_len = read_and_remove_zeros(
        filename, MONSTER_NUM=56
    )
    deleted1 = []
    if do_remove_duplicate_subsequences:
        newdata, deleted1 = do_duplicate(newdata)
    newdata, deleted2, deleted3 = del_duplicate_by_time(
        newdata, delete_no_time
    )
    if not delete_no_time:
        deleted2 = []
...
def create_gui():
    ...
    folder_frame = ttk.LabelFrame(
        root,
        text="处理文件夹(处理文件夹及其所有子文件夹下的CSV文件，并合并为一个)",
    )
    ...
    ttk.Checkbutton(
        folder_frame, text="删除无时间戳数据", variable=del_time
    ...
    # 处理文件的组件
    ttk.Label(file_frame, text="选择文件:").grid(
        row=0, column=0, padx=5, sticky="w"
    )
    ...
    ttk.Label(file_frame, text="保存路径:").grid(
        row=1, column=0, padx=5, sticky="w"
    )
    ...
    ttk.Checkbutton(
        file_frame, text="删除无时间戳数据", variable=del_time_file
    ).grid(row=3, column=0, columnspan=3, sticky="w")
    ...
    ttk.Entry(
        file_frame, textvariable=matched_threshold_file_var, width=6
