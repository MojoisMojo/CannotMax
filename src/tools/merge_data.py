...
def process_archives(merge_images=True, extract_result_images=False):
    ...
    if target_csv_path.exists():
        print(
            f"检测到已存在的 {target_csv_path.name}，正在读取历史记录构建索引..."
        )
        try:
            with open(
                target_csv_path, "r", encoding="utf-8-sig", newline=""
            ) as f:
                reader = csv.reader(f)
                try:
                    header = next(reader)
                    if header == expected_header:
                        ...
                        print(
                            f"-> 成功加载 {len(seen_csv_img_paths)} 条历史记录的索引。"
                        )
                    else:
                        print(
                            "-> 警告：已存在 CSV 的表头与配置不符，将创建新文件或覆写。"
                        )
    ...
    for zip_path in zip_files:
        ...
        try:
            with zipfile.ZipFile(zip_path, "r") as zf:
                ...
                # ==========================================
                csv_members = [
                    m for m in zip_namelist_set if m.endswith("arknights.csv")
                ]
                ...
                for csv_member in csv_members:
                    ...
                    print(
                        f"     CSV: 新增 {len(zip_new_csv_rows)} 条，重复跳过 {skip_csv_count} 条"
                    )
                ...
                if merge_images:
                    for member in zip_namelist_set:
                        ...
                        if member.lower().endswith(tuple(possible_extensions)):
                            ...
                            is_result_img = filename.rsplit(".", 1)[
                                0
                            ].endswith("_result")
                            ...
                            if target_img_path.exists():
                                ...
                                try:
                                    ...
                                    print(
                                        f"  [错误] 提取图片 {member} 失败: {e}"
                                    )

                    print(
                        f"     IMG: 提取新图 {zip_extracted_img_count} 张，已存在跳过 {zip_skip_img_count} 张"
                    )
...
if __name__ == "__main__":
    ...
    process_archives(
        merge_images=merge_imgs, extract_result_images=extract_res_imgs
    )
